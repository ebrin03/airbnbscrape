...
@app.route("/data")
def hello_world():
    currency = request.args.get("currency") or "GBP"
    check_in = request.args.get("check_in") or "2025-09-10"
    check_out = request.args.get("check_out") or "2025-09-14"
    ne_lat = request.args.get("ne_lat") or 47.55
    ne_long = request.args.get("ne_long") or 19.10
    sw_lat = request.args.get("sw_lat") or 47.48
    sw_long = request.args.get("sw_long") or 19.02
    zoom_value = request.args.get("zoom_value") or 2
    price_min = request.args.get("price_min") or 0
    price_max = request.args.get("price_max") or 0
    place_type = request.args.get("place_type") or "Entire home/apt"
    amenities = request.args.get("amenities") or []
    free_cancellation = request.args.get("free_cancellation") or False
    language = request.args.get("language") or "en"
    proxy_url = request.args.get("proxy_url") or ""
